...
double calERR(vector<Tuple>& data, vector<double>& pred, int size,
  double A, MatrixXd& BU, MatrixXd& BI, MatrixXd& BM) {
    double err = 0;
...
void saveModel(double A, MatrixXd& BU, MatrixXd& BI, MatrixXd& BM) {
    /* Meta Information
     * NU NI 
     * A (1, 1)
     * BU (NU, 1)
...
int main() {
    ...
    predict(tn_tuple, tn_p, NTN, A, BU, BI, BM);
    predict(tt_tuple, tt_p, NTT, A, BU, BI, BM);

    double last_tn_err = calERR(tn_tuple, tn_p, NTN, A, BU, BI, BM);
    for (int time = 0; time < MAX_ITER; time++) {
        ...
        if (time % 4 == 0) {
            ...
            double tn_err = calERR(tn_tuple, tn_p, NTN, A, BU, BI, BM);
            double tt_err = calERR(tt_tuple, tt_p, NTT, A, BU, BI, BM);
